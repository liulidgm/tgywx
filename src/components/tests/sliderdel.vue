<template>
    <div class="left-delete">
       ...
       <div class="deleteIcon" :style="zIndex" :id="index"  @click.prevent="deleteItem(taskId,orderId)">删除</div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            _touchStart: function(ev) {
                ...
                //console.log(this.orderId+","+this.taskId)
                ev = ev || event;
...
//如果是向左滑动，则实时给这个根元素一个向左的偏移-left，当偏移量到达固定值delWidth时，固定元素的偏移量为 delWidth
                        this.txtStyle = "transform:translateX(-4rem)";
            ...
            _touchEnd: function(ev) {
                ...
                if (event.changedTouches.length == 1) {
                  ...
                  //  console.log(event.changedTouches[0].clientX)
                    ...
                    console.log();
                    var tx = $("#"+this.index).prev().css('transform').replace(/[^0-9\-,]/g,'').split(',')[4];
                    if(tx==0){
            ...
            deleteItem: function(taskId,orderId) {
                this.$emit('deleteItem', taskId,orderId);
            }
        }
    }
</script>
